    ...
    public class FrooxEngineRunner
    {
        ...
        public static string GetResoniteExePath(out Dictionary<string, Assembly> libraries, string[] args)
        {
            ...
            while (!success)
            {
                ...
                // Native dependencies are distributed separately from the managed assemblies.
                string nativeFolder = Path.Combine(resoniteFolder, "runtimes", RuntimeInformation.RuntimeIdentifier, "native");
                AssemblyLoadContext.Default.ResolvingUnmanagedDll += (assembly, name) =>
                {
                    string[] names = OperatingSystem.IsWindows()
                        ? new[] { name, name + ".dll" }
                        : OperatingSystem.IsMacOS()
                            ? new[] { name, "lib" + name + ".dylib" }
                            : new[] { name, "lib" + name + ".so", name + ".so" };
                    foreach (string directory in new[] { nativeFolder, resoniteFolder })
                        foreach (string candidate in names)
                            if (NativeLibrary.TryLoad(Path.Combine(directory, candidate), out IntPtr handle))
                                return handle;
                    return IntPtr.Zero;
                };
                ...
                Environment.SetEnvironmentVariable("PATH",
                    ...
                    + libraryFolder + Path.PathSeparator
                ...
                AppDomain.CurrentDomain.AssemblyResolve += delegate (object sender, ResolveEventArgs args)
                {
                    ...
                    try
                    {
                        return AssemblyLoadContext.Default.LoadFromAssemblyPath(targetPath);
                    }
                    catch (Exception)
                    {
                        return null;
        ...
        public FrooxEngineRunner(string[] args = null)
        {
            ...
            var outputDeviceProperty = launchOptions.GetType().GetProperty("OutputDevice");
            outputDeviceProperty.SetValue(launchOptions, Enum.Parse(outputDeviceProperty.PropertyType, "Headless"));

            // RuntimeData is shipped engine content, not writable user data.
            string stateDirectory = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData),
                "ResoniteUnityExporterStandalone");
            Directory.CreateDirectory(stateDirectory);
            SetProperty(launchOptions, "DataDirectory", Path.Combine(stateDirectory, "Data"));
            SetProperty(launchOptions, "CacheDirectory", Path.Combine(stateDirectory, "Cache"));
            SetProperty(launchOptions, "LogsDirectory", Path.Combine(stateDirectory, "Logs"));
            ...
            var tmpDir = Directory.CreateTempSubdirectory("ResoniteUnityExporter-");
            ...
            System.Threading.Tasks.Task task = (System.Threading.Tasks.Task)
                CallMethod(engine, "Initialize",
                    libraryPath,
                    false, // Standalone conversion does not need a renderer process.
                    launchOptions,
                    systemInfo,
            ...
            configuredTaskAwaiter.GetResult(); // Surface initialization failures before creating a world.
            // remove tmp dir
            ...
            //}
            CallMethod(engine, "InitializeUpdateLoop");
            ...
            Delegate initWorldDelegate = 
                ...
                delegate (object world)
                {
                    ...
                    mainRootSlot = CallMethod(world, "AddSlot", "ResoniteDataWrapperSlotRoot", true);
                });
            ...
            new Thread(() =>
            {
                ...
                try
                {
                    ...
                    while (!opener.IsCompleted || mainRootSlot == null)
                    {
                        if (opener.IsFaulted || opener.IsCanceled) opener.GetAwaiter().GetResult();
                        CallMethod(engine, "RunUpdateLoop");
                        CallMethod(systemInfo, "FrameFinished");
                        Thread.Sleep(16);
                    }
                    opener.GetAwaiter().GetResult();
                    CallMethod(GetProperty(engine, "WorldManager"), "FocusWorld", GetProperty(mainRootSlot, "World"));
                    ...
                    while (true)
                    {
                        ...
                        if (focusedWorld != null)
                        {
                            if (first)
                            {
                                ...
                                Console.WriteLine("Starting IPC server in " + serverDirectory);
                                bridgeServer = new ResoniteBridgeLib.ResoniteBridgeServer("UnityResoniteImporter", serverDirectory, (msg) => Console.WriteLine("IPC: " + msg));
                                Console.WriteLine("IPC server created; registering import handlers");
                                ...
                                readyToProcess.Set();
                                Console.WriteLine("Standalone exporter ready for Unity connections");
                            }   
                        }
                        try
                        {
